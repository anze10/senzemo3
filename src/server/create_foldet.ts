...
async function createFolder(client: OAuth2Client, customer_name: string, order_number: string) {
    ...
    try {
        ...
        const headerFormattingRequest = {
            repeatCell: {
                ...
                fields: 'userEnteredFormat(backgroundColor,borders,textFormat,horizontalAlignment,verticalAlignment)',
            },
        };
        ...
        await sheets.spreadsheets.batchUpdate({
            spreadsheetId: spreadsheetId !== null ? spreadsheetId : undefined,
            requestBody: {
                requests: [...boldRightAlignRequests, headerFormattingRequest, resizeColumnsRequests, mergeCellsRequest, imageAlignmentRequest, imageRequests],
            },
        });
...
async function createSpreadsheetCsv(client: OAuth2Client, folderId: string | null | undefined, order_number: string) {
    const service = google.drive({ version: 'v3', auth: client as any });
    const fileMetadata = {
...
export async function createFolderAndSpreadsheet(customer_name: string, order_number: string) {
    ...
    try {
        ...
        const spreadsheetId = await createSpreadsheet(client, folderId, customer_name, order_number, currentTime, name as string);;
